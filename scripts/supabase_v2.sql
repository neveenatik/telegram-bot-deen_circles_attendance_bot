...
-- weekly slot: a session type held on a given weekday at a given time, with an
-- optional linked teacher. This is a plan/timetable only — it does NOT create
-- attendance sessions. Rows carry a stable id so a future scheduled-actions
-- feature can reference a slot to fire timed reminders relative to its day/time.
