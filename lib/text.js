...
export const TEXT = {
  ...
  secondaryReport: (session) => {
    ...
    const attendance = session.attendance || {};
    const byStatus = { present: [], listening: [], excused: [], absent: [] };

    for (const [name, statusRaw] of Object.entries(attendance)) {
      const status = statusRaw || 'absent';
      const verse = verses[name] || '—';
      if (!byStatus[status]) byStatus.absent.push([name, verse]);
      else byStatus[status].push([name, verse]);
    }

    const sortByName = (arr) => [...arr].sort((a, b) => compareArabic(a[0], b[0]));
    byStatus.present = sortByName(byStatus.present);
    byStatus.listening = sortByName(byStatus.listening);
    byStatus.excused = sortByName(byStatus.excused);
    byStatus.absent = sortByName(byStatus.absent);

    const total = byStatus.present.length + byStatus.listening.length + byStatus.excused.length + byStatus.absent.length;
    if (!total) return `🧾 *تقرير حلقة تصحيح التلاوة "${session.name}":*\n\nلم يُسجّل أحد.`;

    let r = `🧾 *تقرير حلقة تصحيح التلاوة "${session.name}" (${total} طالبة):*\n\n`;
    const withVerse = (arr) => arr.map(([n, v]) => `${n} — ${v}`).join('\n');

    if (byStatus.present.length) r += `✅ *حاضرة (${byStatus.present.length}):*\n${withVerse(byStatus.present)}\n\n`;
    if (byStatus.listening.length) r += `👂 *مستمعة (${byStatus.listening.length}):*\n${withVerse(byStatus.listening)}\n\n`;
    if (byStatus.excused.length) r += `🔔 *معتذرة (${byStatus.excused.length}):*\n${withVerse(byStatus.excused)}\n\n`;
    if (byStatus.absent.length) r += `❌ *غياب (${byStatus.absent.length}):*\n${withVerse(byStatus.absent)}`;
