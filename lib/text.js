...
export const TEXT = {
  ...
  report: (session, groups) => {
    ...
    if (groups.present.length) r += `✅ *حاضرة (${groups.present.length}):*\n${sortNamesAlphabetically(groups.present).map((n) => participants.label(session, n)).join('\n')}\n\n`;
    if (groups.listening.length) r += `👂 *مستمعة فقط (${groups.listening.length}):*\n${sortNamesAlphabetically(groups.listening).map((n) => participants.label(session, n)).join('\n')}\n\n`;
    if (groups.excused.length) r += `🔔 *معتذرة (${groups.excused.length}):*\n${sortNamesAlphabetically(groups.excused).map((n) => participants.label(session, n)).join('\n')}\n\n`;
    // Absent is reported only for admin-authorized (roster-attendance) lists.
    if (requiresRegistrationApproval(session.type) && groups.absent.length) r += `❌ *غياب (${groups.absent.length}):*\n${sortNamesAlphabetically(groups.absent).map((n) => participants.label(session, n)).join('\n')}`;
    return r;
  },
  secondaryReport: (session) => {
    ...
    const lines = present.map(([name, verse]) => `${participants.label(session, name)} — ${verse}`).join('\n');
