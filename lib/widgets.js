...
// Stable per-participant callback token for the manage widget: the Telegram
// userId for registered members (immune to renames / row reordering) and a
// positional index for guests (who have no userId). Resolved by
// resolveManageTarget in the manage action handlers.
export function manageMemberToken(session, name, displayIndex) {
  const uid = participants.getMemberId(session, name);
...
export const sessionKb = (session) => {
  ...
  if (!active || !registrationOpen) return Markup.inlineKeyboard([]);
  const adminRow = [
    Markup.button.callback(TEXT.refreshButton, 'a:refresh'),
    Markup.button.callback(TEXT.editListButton, 'a:edit'),
    Markup.button.callback(TEXT.stopListButton, 'a:stop'),
  ];
  // Recitation-correction lists gate self-registration behind a three-part
  // attestation shown in the widget footer: the student taps one confirm button
  // (a:recite) which registers her present (see attendance.recite).
  if (session?.type === 'registeredSecondary') {
...
export function manageKb(session, master, page = 0, sessionType = null) {
  ...
  const rows = paged.map((name, i) => {
    ...
    return [Markup.button.callback(`${e} ${name}`, scopedManageCallback(session, sessionType, 'pick', manageMemberToken(session, name, start + i)))];
  });
