...
export const sessionKb = (active, registrationOpen = true, openRegistration = false) =>
  active && registrationOpen
    ? Markup.inlineKeyboard([[
        ...
        ...(!openRegistration ? [Markup.button.callback(TEXT.statusButtons.excused, 'a:excused')] : []),
      ]])
    : Markup.inlineKeyboard([]);
...
export async function refreshSessionWidget(telegram, session, master) {
  ...
  try {
    await telegram.editMessageText(
      ...
      { parse_mode: 'Markdown', ...sessionKb(session.active, session.registrationOpen !== false, session.openRegistration) }
    );
  } catch (e) {
