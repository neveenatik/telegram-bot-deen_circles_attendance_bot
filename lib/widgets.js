...
function sessionSummaryLines(session, master) {
  ...
  return [
    ...
    `✅ حاضرة: ${summary.present}`,
    `👂 مستمعة: ${summary.listening}`,
    `🔔 معتذرة: ${summary.excused}`,
    `⏳ لم تُسجّل: ${summary.pending}`,
  ];
}
...
function buildSessionListTexts(session, master, maxLen = 3500) {
  ...
  if (!lines.length) {
    return [`📋 *متابعة الحضور — ${session.name}*\n\n— لا توجد أسماء حتى الآن —`];
  }
  ...
  return chunkBodies.map((body, i) => `📋 *متابعة الحضور — ${session.name}* (${i + 1}/${total})\n\n${body}`);
}
...
export function sessionText(session, master) {
  ...
  const listInfo = Array.isArray(session.listMessageIds) && session.listMessageIds.length
    ? `📄 رسائل القائمة: ${session.listMessageIds.length}`
    : '📄 رسائل القائمة: 1';
  ...
  for (let i = 0; i < common; i += 1) {
    try {
      ...
      if (!e.message?.includes('message is not modified')) {
        console.error('syncSessionListMessages edit:', e.message);
      }
    }
  }
  ...
  if (texts.length > existing.length) {
    for (let i = existing.length; i < texts.length; i += 1) {
      try {
        ...
        console.error('syncSessionListMessages send:', e.message);
      }
    }
  }
...
export async function refreshSessionWidget(telegram, session, master, persistSession) {
  ...
  if (changed && typeof persistSession === 'function') {
    try {
      ...
      console.error('refreshSessionWidget persistSession:', e?.message || e);
    }
  }
}

export async function refreshManageWidget(ctx, session, master, page = 0) {
  try {
    await ctx.editMessageText(manageText(session, master, page), { parse_mode: 'Markdown', ...manageKb(session, master, page) });
  } catch (e) {
...
export function membersKb(master, page = 0) {
  ...
  const rows = paged.map((name, i) => [
    Markup.button.callback(`🔧 ${name}`, `mb:pick:${start + i}`),
  ]);

  if (totalPages > 1) {
    rows.push([
      ...(safePage > 0 ? [Markup.button.callback('⬅️', `mb:page:${safePage - 1}`)] : []),
      Markup.button.callback(`📄 ${safePage + 1}/${totalPages}`, 'mb:noop'),
      ...(safePage < totalPages - 1 ? [Markup.button.callback('➡️', `mb:page:${safePage + 1}`)] : []),
    ]);
  }
...
export function memberOptionsKb(idx, page = 0) {
...
export function manageKb(session, master, page = 0) {
  const names = sessionNames(session, master);
  const totalPages = Math.max(1, Math.ceil(names.length / MANAGE_PAGE_SIZE));
  const safePage = clampPage(page, totalPages);
  const start = safePage * MANAGE_PAGE_SIZE;
  ...
  const rows = paged.map((name, i) => {
    ...
    return [Markup.button.callback(`${e} ${name}`, `sm:pick:${start + i}`)];
  });

  if (totalPages > 1) {
    rows.push([
      ...(safePage > 0 ? [Markup.button.callback('⬅️', `sm:page:${safePage - 1}`)] : []),
      Markup.button.callback(`📄 ${safePage + 1}/${totalPages}`, 'sm:noop'),
      ...(safePage < totalPages - 1 ? [Markup.button.callback('➡️', `sm:page:${safePage + 1}`)] : []),
    ]);
  }

  rows.push([
    Markup.button.callback(TEXT.manageButtons.addGuest, 'sm:addguest'),
    Markup.button.callback(TEXT.manageButtons.editSessionName, `sm:editname:${safePage}`),
  ]);
  rows.push([
    Markup.button.callback(TEXT.refreshButton, `sm:refresh:${safePage}`),
