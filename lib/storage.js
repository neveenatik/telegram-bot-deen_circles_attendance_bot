...
async function supabaseBackend() {
  ...
  return {
    ...
    listScheduleSlots: async (groupId) => {
      ...
      const { data, error } = await db
        .from('class_schedule')
        .select('id,session_type,day_of_week,time_of_day,teacher_id,teachers:teacher_id (id,name,teacher_type)')
        .eq('group_id', groupRowId)
        .eq('active', true)
      ...
      return (data || []).map((row) => ({
        ...
        teacherType: row.teachers?.teacher_type ?? null,
      }));
    },
    getScheduleSlot: async (groupId, slotId) => {
      ...
      return {
        ...
        teacherType: data.teachers?.teacher_type ?? null,
      };
    },
    ...
    listScheduleForUser: async (userId) => {
      ...
      for (const r of tzRows || []) tzById.set(r.group_id, r.timezone || 'Asia/Riyadh');
      ...
      return (data || []).map((row) => ({
        ...
        timeOfDay: row.time_of_day,
        teacherName: row.teachers?.name ?? null,
        timezone: tzById.get(row.group_id) || 'Asia/Riyadh',
      }));
    },
    ...
    getClassTimezone: async (groupId) => {
      const gid = normalizeGroupId(groupId);
      const groupRowId = await findGroupRowId(gid);
      if (!groupRowId) return 'Asia/Riyadh';
      const { data, error } = await db
        .from('group_settings')
        .select('timezone')
        .eq('group_id', groupRowId)
