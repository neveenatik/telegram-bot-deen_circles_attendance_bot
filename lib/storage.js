...
async function supabaseBackend() {
  ...
  const upsertParticipant = async (sessionId, groupRowId, session, name) => {
    ...
    const onConflict = memberId ? 'session_id,member_id' : 'session_id,guest_name';
  ...
  /** Delete a SINGLE participant row by display name (granular remove). */
  const deleteParticipant = async (sessionId, name) => {
  ...
  const upsertSessionRow = async (gid, type, session, archived = false) => {
    ...
    // Strip participant data — it is stored in session_participants, not the blob
    const { participants, ...blobData } =
  ...
  return {
    ...
    // Granular writes: persist the session row (metadata blob) plus a SINGLE participant
    // row, instead of rewriting every participant. Use on hot paths (attendance clicks,
    // status changes) to avoid the full-rewrite cost and the last-writer-wins race.
    ...
    removeParticipant: async (groupId, type, s, name) => {
      const gid = normalizeGroupId(groupId);
      await ensureGroupRowId(gid);
      const sessionId = await upsertSessionRow(gid, type, s, false);
      await deleteParticipant(sessionId, name);
    },
