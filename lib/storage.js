...
const TRAINING_GROUPS_FILE         = path.join(DATA_DIR, 'trainingGroups.json');
...
const pruneTrainingGroupRefsFromFileMap = (groupId) => {
  const gid = normalizeGroupId(groupId);
  const all = readMap(TRAINING_GROUPS_FILE);
  let changed = false;

  for (const [ownerGroupId, rows] of Object.entries(all)) {
    if (!Array.isArray(rows)) continue;
    const next = rows.filter((row) => String(row?.groupId || '') !== gid);
    if (next.length !== rows.length) {
      all[ownerGroupId] = next;
      changed = true;
    }
  }

  if (changed) writeJSON(TRAINING_GROUPS_FILE, all);
};
...
const fileBackend = {
  ...
  getTrainingGroups: async (groupId) => {
    const gid = normalizeGroupId(groupId);
    const all = readMap(TRAINING_GROUPS_FILE);
    const rows = all[gid] && Array.isArray(all[gid]) ? all[gid] : [];
    return rows
  ...
  saveTrainingGroups: async (groupId, groups) => {
    const gid = normalizeGroupId(groupId);
    const all = readMap(TRAINING_GROUPS_FILE);
    all[gid] = Array.isArray(groups)
      ? groups
          .filter((row) => row?.groupId && row?.name)
          .map((row) => ({ groupId: String(row.groupId), name: String(row.name) }))
      : [];
    writeJSON(TRAINING_GROUPS_FILE, all);
  },
  ...
  clearGroupData: async (groupId) => {
    ...
    deleteGroupFromFileMap(TRAINING_GROUPS_FILE, groupId);
    deleteGroupFromFileMap(ACTIVITY_FILE, groupId);
    deleteGroupFromFileMap(PENDING_REG_FILE, groupId);
    pruneTrainingGroupRefsFromFileMap(groupId);
  },
};
...
async function supabaseBackend() {
  ...
  const loadParticipantsForSessions = async (sessionIds, groupRowId) => {
    ...
    const [{ data: members, error: membersError }, { data: participants, error: participantsError }] = await Promise.all([
    ...
    for (const sid of sessionIds) {
      const sessionParts = (participants || []).filter((p) => p.session_id === sid);
      ...
      for (const member of (members || [])) {
        ...
        attendance[name] = p ? mapAttendanceStatus(p.attendance_status) : null;
        called[name] = p ? mapCalledState(p.called_state) : null;
        if (p?.pages) pages[name] = p.pages;
        if (p?.verse) verses[name] = p.verse;
        if (p?.registration_time) registrationTimes[name] = new Date(p.registration_time).getTime();
        memberIds[name] = member.telegram_user_id;
      }

      // Guests
      for (const p of guestParts) {
        const name = p.display_name;
        attendance[name] = mapAttendanceStatus(p.attendance_status);
        called[name] = mapCalledState(p.called_state);
        if (p.pages) pages[name] = p.pages;
        if (p.verse) verses[name] = p.verse;
        if (p.registration_time) registrationTimes[name] = new Date(p.registration_time).getTime();
      }

      result[sid] = { attendance, called, pages, verses, registrationTimes, memberIds };
    }
  ...
  const toLegacySessionBlob = (row, participantData = null) => {
    ...
    if (participantData) {
      return {
        ...
        called: participantData.called,
        pages: participantData.pages,
        verses: participantData.verses,
        registrationTimes: participantData.registrationTimes,
        memberIds: { ...(base.memberIds || {}), ...participantData.memberIds },
      };
    }
  ...
  const upsertSessionParticipants = async (sessionId, groupRowId, session) => {
    ...
    for (const name of allNames) {
      const uid = session.memberIds?.[name] ? String(session.memberIds[name]) : null;
      const memberId = uid ? (uidToMemberId.get(uid) ?? null) : null;
      const regTime = session.registrationTimes?.[name];
      rows.push({
        session_id: sessionId,
        member_id: memberId,
        guest_name: memberId ? null : name,
        display_name: name,
        attendance_status: mapAttendanceStatus(session.attendance?.[name]),
        called_state: mapCalledState(session.called?.[name]),
        registration_time: regTime ? new Date(regTime).toISOString() : null,
        pages: session.pages?.[name] != null ? String(session.pages[name]) : null,
        verse: session.verses?.[name] || null,
      });
    }
  ...
  const upsertSessionRow = async (gid, type, session, archived = false) => {
    ...
    // Strip participant fields — they are stored in session_participants, not the blob
    const { attendance, called, pages, verses, registrationTimes, memberIds, ...blobData } =
  ...
  return {
    ...
    getSession:     async (groupId, type)  => {
      ...
      const { data, error } = await db
        .from('sessions')
        .select('id,session_type,name,series_id,active,chat_id,widget_message_id,started_at,ended_at,archived,metadata')
        .eq('group_id', groupRowId)
