...
async function supabaseBackend() {
  ...
  const loadParticipantsForSessions = async (sessionIds, groupRowId) => {
    ...
    const [members, participantRows] = await Promise.all([
      ...
      fetchAllRows(() => db.from('session_participants')
        .select('session_id, member_id, guest_name, display_name, attendance_status, called_state, registration_time, pages, verse, attended_main, backup')
  ...
  return {
    ...
    // Return the teacher assigned to a session ({ id, name, type }) or null.
    getSessionTeacher: async (sessionId) => {
      if (!sessionId) return null;
      const { data, error } = await db
        .from('sessions')
        .select('teacher_id, teachers:teacher_id (id,name,teacher_type)')
        .eq('id', sessionId)
        .maybeSingle();
      if (error) throw error;
      const t = data?.teachers;
      if (!t) return null;
      return { id: t.id, name: t.name, type: t.teacher_type };
    },
    getTeachers: async (groupId) => {
      ...
      const { data, error } = await db
        .from('teachers')
        .select('id,telegram_user_id,name,teacher_type')
        .eq('group_id', groupRowId)
        .eq('active', true)
        .order('created_at', { ascending: true });
      if (error) throw error;
    ...
    // Teaching materials: we persist only Telegram's file_id (Telegram hosts the
    // bytes) plus a human title; the same file_id is later reused to resend the
    // file. Soft-deleted (active=false) like teachers so history stays intact.
    getMaterials: async (groupId) => {
      ...
      const { data, error } = await db
        ...
        .select('id,title,file_id,file_type,file_name,added_by,created_at')
        .eq('group_id', groupRowId)
        .eq('active', true)
        .order('created_at', { ascending: true });
      if (error) throw error;
    ...
    addMaterial: async (groupId, material) => {
      ...
      const payload = {
        ...
        file_id: String(material.fileId),
        file_type: String(material.fileType),
        file_name: material.fileName ? String(material.fileName) : null,
    ...
    // keeps the earliest submitted_at but refreshes the message pointer.
    recordSubmission: async (homeworkId, memberId, submissionMessageId = null) => {
      ...
      if (existing?.id) {
        if (submissionMessageId !== null) {
          const { error: updateError } = await db
            .from('homework_submissions')
            .update({ submission_message_id: submissionMessageId })
            .eq('id', existing.id);
          if (updateError) throw updateError;
        }
        return existing.id;
    ...
    // to the student's submission). Returns true when a row was updated.
    ...
    getSubmissions: async (homeworkId) => {
      const { data, error } = await db
        .from('homework_submissions')
        .select('id,member_id,submission_message_id,submitted_at,reviewed,reviewed_by,reviewed_at,members(name)')
        .eq('homework_id', homeworkId)
    ...
    addOfflineTeachers: async (groupId, entries) => {
      ...
      const rows = (Array.isArray(entries) ? entries : [])
        ...
        .map((e) => ({
          ...
          teacher_type: String(e.type).trim(),
          active: true,
        }))
        .filter((r) => r.name && r.teacher_type);
    ...
    // Change an offline teacher's assigned type (courseteacher/trainingteacher/…).
    setOfflineTeacherType: async (groupId, teacherId, teacherType) => {
      const gid = normalizeGroupId(groupId);
      const type = String(teacherType || '').trim();
      const tid = Number(teacherId);
      if (!type || !Number.isInteger(tid)) return { ok: false };
      const groupRowId = await ensureGroupRowId(gid);
      const { data, error } = await db
        .from('teachers')
        .update({ teacher_type: type })
        .eq('group_id', groupRowId)
        .eq('id', tid)
        .eq('active', true)
        .select('name,teacher_type');
      if (error) throw error;
      if (!data || !data.length) return { ok: false };
      return { ok: true, name: data[0].name, type: data[0].teacher_type };
    },
