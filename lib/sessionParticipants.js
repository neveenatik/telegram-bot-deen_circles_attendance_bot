...
export function get(session, name) {
  ...
  if (rec.pendingApproval !== undefined) view.pendingApproval = rec.pendingApproval;
...
/**
 * Participant names ordered by roster list number (ascending). Members without a
 * list number (e.g. walk-in guests) sort last, alphabetically. Used by widgets
 * and reports that should mirror the roster order rather than the alphabet.
 */
export function namesByListNumber(session) {
  return [...names(session)].sort((a, b) => {
    const la = getListNumber(session, a);
    const lb = getListNumber(session, b);
    if (la == null && lb == null) return a.localeCompare(b, 'ar');
    if (la == null) return 1;
    if (lb == null) return -1;
    return la - lb;
  });
...
// A walk-in counted present live but still awaiting admin approval (queued in the
// pending-registrations panel). Rendered with a ⏳ tag in the live list; cleared
// once she is approved (becomes a roster member) or dismissed.
export function setPendingApproval(session, name, value) {
  ensureRecord(session, name).pendingApproval = value ? true : false;
}
...
export function memberToken(session, name, displayIndex) {
  ...
  // Only numeric Telegram ids are safe inside a colon-delimited callback and the
  // `u<digits>` route pattern. Offline members carry a synthetic non-numeric id
  // (`offline:<uuid>`), so they fall back to a positional token like guests.
  return uid && /^\d+$/.test(uid) ? `u${uid}` : `g${displayIndex}`;
}
