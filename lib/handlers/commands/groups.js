...
export function register(bot, storage) {
  const { getMaster, saveMaster, getTrainingGroups, saveTrainingGroups } = storage;

  bot.command('listtrainingstudents', async (ctx) => {
  ...
  bot.command('addtraininggroup', async (ctx) => {
    if (!await isAdmin(ctx)) return replyEphemeral(ctx, TEXT.adminOnly);
  ...
  bot.command('removetraininggroup', async (ctx) => {
    if (!await isAdmin(ctx)) return replyEphemeral(ctx, TEXT.adminOnly);
  ...
  bot.command('listtraininggroups', async (ctx) => {
    if (!await isAdmin(ctx)) return replyEphemeral(ctx, TEXT.adminOnly);
