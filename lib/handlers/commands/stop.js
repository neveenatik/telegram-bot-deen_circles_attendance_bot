...
export function register(bot, storage) {
  ...
  bot.command('stopregistration', async (ctx) => {
    ...
    const active = await getActiveSessionContext(groupId);
    if (!active) return replyEphemeral(ctx, TEXT.noSessionActive);
    ...
    try {
      ...
      console.warn('stopregistration refresh failed:', err?.message || err);
    }
