...
import { SESSION_TYPES, getActiveSessionType } from '../../sessionTypes.js';
 ...
 */
export async function sendSessionReport(ctx, session, master, pending = [], options = {}) {
  ...
  const teacherLine = options.teacherName
    ? `${TEXT.sessionReportTeacherLine(options.teacherName)}\n\n`
    : '';

  if (session.type === 'personalRecitation') {
    await replyChunked(ctx, teacherLine + TEXT.pageListReport(session, opts), { parse_mode: 'Markdown' });
  } else if (session.type === 'groupRecitation') {
    await replyChunked(ctx, teacherLine + TEXT.groupRecitationReport(session, opts), { parse_mode: 'Markdown' });
  } else if (session.type === 'registeredSecondary') {
    await replyChunked(ctx, teacherLine + TEXT.secondaryReport(session, opts), { parse_mode: 'Markdown' });
  } else {
    await replyChunked(ctx, teacherLine + TEXT.report(session, groups, opts), { parse_mode: 'Markdown' });
  }
}
  ...
  async function stoplist(ctx) {
    ...
    for (const { type: activeType, session } of activeSessions) {
      try {
        ...
        // Remove the live pending-requests group nudge (if any) as the session ends.
        if (session.pendingNudgeMessageId) {
          try {
            await telegram.deleteMessage(groupId, session.pendingNudgeMessageId);
          } catch (err) {
            logTelegramError('stop.deletePendingNudge', err, {
              groupId,
              sessionType: activeType,
              messageId: session.pendingNudgeMessageId,
            });
          }
          session.pendingNudgeMessageId = null;
        }
