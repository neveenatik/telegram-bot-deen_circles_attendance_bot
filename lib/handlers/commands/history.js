  ...
  async function classhistory(ctx) {
    ...
    try {
      await telegram.sendMessage(ctx.from.id, TEXT.historyHomeText(querySeries, scoped.length), {
        ...
        reply_markup: {
          inline_keyboard: [
            [{ text: TEXT.historyShowReportButton, callback_data: `h:rep:${groupId}:${querySeries}` }],
            [{ text: TEXT.historyEditSessionsButton, callback_data: `h:edit:${groupId}:${querySeries}:0` }],
