...
export function register(bot, storage) {
  const { getMaster, getSession, getAllSessions, getCurrentSeries, getTrainingGroups } = storage;
  ...
  bot.command('classhistory', async (ctx) => {
    ...
    return ctx.replyWithMarkdown(TEXT.historyHomeText(querySeries, scoped.length), {
      reply_markup: {
        inline_keyboard: [
          [{ text: TEXT.historyShowReportButton, callback_data: `h:rep:${querySeries}` }],
          [{ text: TEXT.historyEditSessionsButton, callback_data: `h:edit:${querySeries}:0` }],
          [{ text: TEXT.closeButton, callback_data: 'msg:dismiss' }],
        ],
      },
    });
  });
