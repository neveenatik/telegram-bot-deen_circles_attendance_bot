...
export function register(bot, storage) {
  ...
  bot.command('classhistory', async (ctx) => {
    ...
    // Check for optional series parameter: /classhistory [series_number]
    const arg = ctx.message.text.split(' ').slice(1).join(' ').trim();
    const querySeries = arg ? parseInt(arg, 10) : currentSeries;
    
    if (arg && (!Number.isInteger(querySeries) || querySeries < 1)) {
      return ctx.reply(TEXT.invalidSeriesNumber);
    }

    const scoped = sessionsInSeries(all, querySeries);
    if (!scoped.length) return ctx.reply(TEXT.noSeriesRecords(querySeries));

    const lines = scoped.map((s, i) => TEXT.recordsLine(i + 1, s));
    return ctx.reply(`${TEXT.recordsHeader(querySeries, scoped.length)}\n\n${lines.join('\n')}`);
  });
  ...
  bot.command('studentshistory', async (ctx) => {
    ...
    // Check for optional series parameter: /studentshistory [series_number]
    const arg = ctx.message.text.split(' ').slice(1).join(' ').trim();
    const querySeries = arg ? parseInt(arg, 10) : currentSeries;
    ...
    const sessions = sessionsInSeries(all, querySeries);
