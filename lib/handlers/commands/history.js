...
export function register(bot, storage) {
  ...
  bot.command('studentshistory', async (ctx) => {
    ...
    const lines = sortArabic(Object.keys(tally)).map((name) => {
      const t = tally[name];
      const latestVerse = typeof latestVerses[name] === 'string' && latestVerses[name].trim() ? latestVerses[name] : '—';
      return TEXT.historyLine(name, t.present, t.listening, t.excused, t.absent, latestVerse);
    });
