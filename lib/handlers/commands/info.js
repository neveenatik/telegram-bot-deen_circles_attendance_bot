...
export function createHandlers({ storage }) {
  ...
  async function start(ctx, next) {
    ...
    if (ctx.chat?.type === 'private') {
      ...
      // A plain /start in DM is anyone opening the bot (member, student, or
      // admin) — welcome them with the help card, never the admin-only error.
      return ctx.replyWithMarkdown(TEXT.help(false));
    }
