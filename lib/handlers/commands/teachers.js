...
export function register(bot, storage) {
  ...
  bot.command('removeteacher', async (ctx) => {
    ...
    if (!name) return ctx.reply(TEXT.invalidRemoveTeacherFormat);

    const teachers = await getTeachers(groupId);
    const idx = teachers.findIndex(t => t.name === name);
    if (idx === -1) return ctx.reply(TEXT.teacherNotFound(name));
  ...
  bot.command('assignteacher', async (ctx) => {
    ...
    if (parts.length !== 2 || !parts[0] || !parts[1])
      return ctx.reply(TEXT.invalidAssignTeacherFormat);
    ...
    if (!teacher) return ctx.reply(TEXT.teacherNotFound(name));
  ...
  bot.command('tagteachers', async (ctx) => {
    ...
    if (!type) {
      return ctx.reply(TEXT.invalidTagTeacherFormat);
    }
    ...
    if (filtered.length === 0) {
      return ctx.reply(TEXT.noTeachersOfType(typeLabel[type]));
    }
