...
export function createHandlers({
  ...
  async function starttraininglist(ctx) {
    ...
    // Training lists are initialized from the training group's own roster.
    // Members are assigned to a training group from the main group via the
    // member-management UI, so the list starts pre-populated with them.
