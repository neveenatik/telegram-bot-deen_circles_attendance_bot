...
export function register(bot, storage) {
  ...
  async function startSession(ctx, type, initializeAttendance) {
    ...
    if (!name) return replyEphemeral(ctx, TEXT.invalidStartFormat);
  ...
  bot.command('startpersonalrecitation', async (ctx) => {
    ...
    const name = ctx.message.text.split(' ').slice(1).join(' ').trim();
    if (!name) return replyEphemeral(ctx, TEXT.invalidPageListFormat);
  ...
  bot.command('startgrouprecitation', async (ctx) => {
    ...
    const name = ctx.message.text.split(' ').slice(1).join(' ').trim();
    if (!name) return replyEphemeral(ctx, TEXT.invalidStartGroupRecitationFormat);
