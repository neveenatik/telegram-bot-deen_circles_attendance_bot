...
export function register(bot, storage) {
  ...
  async function refreshFirstActiveSession(groupId, requestedType) {
    for (const type of ACTIVE_SESSION_TYPES) {
      ...
      console.log(JSON.stringify({
        level: 'info',
        event: 'start_refresh_existing_session',
        groupId,
        requestedType,
        activeType: type,
  ...
  async function startSession(ctx, type, initializeAttendance, defaultName) {
    ...
    if (await hasActiveSession(groupId)) {
      ...
      await refreshFirstActiveSession(groupId, type);
  ...
  bot.command('startpersonalrecitation', async (ctx) => {
    ...
    if (await hasActiveSession(groupId)) {
      ...
      await refreshFirstActiveSession(groupId, 'personalRecitation');
      return replyEphemeral(ctx, TEXT.refreshed);
    }
  ...
  bot.command('startgrouprecitation', async (ctx) => {
    ...
    if (await hasActiveSession(groupId)) {
      ...
      await refreshFirstActiveSession(groupId, 'groupRecitation');
      return replyEphemeral(ctx, TEXT.refreshed);
    }
