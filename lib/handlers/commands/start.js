...
export function createHandlers({
  ...
  async function starttraininglist(ctx) {
    ...
    // Training lists are pre-populated from the training group's assigned roster
    // (members are assigned from the main group via the member-management UI),
    // but also allow public registration so a walk-in / unregistered student can
    // tap "present". Walk-ins are counted present but queued as pending students
    // for teacher approval (an accidental tap must not add them to the roster);
    // approving them adds them here and backfills them into the main group.
    return startSession(ctx, 'main', true, DEFAULT_SESSION_NAMES.training, { allowPublicRegistration: true });
  }
