...
export function register(bot, storage) {
  ...
  bot.action(/^mb:atrain:(\d+):(\d+)$/, h.assign);
  bot.action(/^mb:atrainunpick:(\d+):(\d+):(-?\d+)$/, h.unassign);
  bot.action(/^mb:atrainpage:(\d+):(\d+):(\d+)$/, h.page);
  bot.action(/^mb:atrainpick:(\d+):(\d+):(-?\d+)$/, h.pick);
}