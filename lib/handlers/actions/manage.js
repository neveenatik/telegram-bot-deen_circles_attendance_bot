...
export function register(bot, storage) {
  ...
  bot.action(/^sm:choose:([a-zA-Z]+)$/, async (ctx) => {
  ...
  bot.action(/^sm:(?:([a-zA-Z]+):)?pick:(\d+)$/, async (ctx) => {
  ...
  bot.action(/^sm:(?:([a-zA-Z]+):)?set:(\d+):(present|listening|excused|absent)$/, async (ctx) => {
    if (!await isAdmin(ctx))
      return ctx.answerCbQuery(TEXT.adminOnly);
  ...
  bot.action(/^sm:(?:([a-zA-Z]+):)?call:(\d+):(responding|responded|away|clear)$/, async (ctx) => {
    if (!await isAdmin(ctx))
      return ctx.answerCbQuery(TEXT.adminOnly);
  ...
  bot.action(/^sm:(?:([a-zA-Z]+):)?editpage:(\d+)$/, async (ctx) => {
    if (!await isAdmin(ctx))
      return ctx.answerCbQuery(TEXT.adminOnly);
  ...
  bot.action(/^sm:(?:([a-zA-Z]+):)?editverse:(\d+)$/, async (ctx) => {
    if (!await isAdmin(ctx))
      return ctx.answerCbQuery(TEXT.adminOnly);
  ...
  bot.action(/^sm:(?:([a-zA-Z]+):)?back:(\d+)$/, async (ctx) => {
    await ctx.answerCbQuery();
    const groupId = groupIdFromCtx(ctx);
    const scopedType = ctx.match[1] || null;
    const activeType = await resolveSessionType(groupId, scopedType);
  ...
  bot.action(/^sm:(?:([a-zA-Z]+):)?back$/, async (ctx) => {
    await ctx.answerCbQuery();
    const groupId = groupIdFromCtx(ctx);
    const scopedType = ctx.match[1] || null;
    const activeType = await resolveSessionType(groupId, scopedType);
  ...
  bot.action(/^sm:(?:([a-zA-Z]+):)?page:(\d+)$/, async (ctx) => {
    await ctx.answerCbQuery();
    const groupId = groupIdFromCtx(ctx);
    const scopedType = ctx.match[1] || null;
    const activeType = await resolveSessionType(groupId, scopedType);
  ...
  bot.action(/^sm:(?:([a-zA-Z]+):)?noop$/, async (ctx) => {
    await ctx.answerCbQuery();
  });

  bot.action(/^sm:(?:([a-zA-Z]+):)?refresh:(\d+)$/, async (ctx) => {
    const groupId = groupIdFromCtx(ctx);
    const scopedType = ctx.match[1] || null;
    const activeType = await resolveSessionType(groupId, scopedType);
    if (!activeType) return ctx.answerCbQuery(TEXT.noSessionShort);
  ...
  bot.action(/^sm:(?:([a-zA-Z]+):)?refresh$/, async (ctx) => {
    const groupId = groupIdFromCtx(ctx);
    const scopedType = ctx.match[1] || null;
    const activeType = await resolveSessionType(groupId, scopedType);
    if (!activeType) return ctx.answerCbQuery(TEXT.noSessionShort);
  ...
  bot.action(/^sm:(?:([a-zA-Z]+):)?hide$/, async (ctx) => {
    if (!await isAdmin(ctx))
      return ctx.answerCbQuery(TEXT.adminOnly);
    await ctx.answerCbQuery();
  ...
  bot.action(/^sm:(?:([a-zA-Z]+):)?addguest$/, async (ctx) => {
    if (!await isAdmin(ctx))
      return ctx.answerCbQuery(TEXT.adminOnly);
  ...
  bot.action(/^sm:(?:([a-zA-Z]+):)?editname:(\d+)$/, async (ctx) => {
    if (!await isAdmin(ctx))
      return ctx.answerCbQuery(TEXT.adminOnly);
