...
export function createHandlers({
  ...
  async function pick(ctx) {
    ...
    const master = await getMaster(groupId);
    const target = resolveManageTarget(session, master, ctx.match[2]);
    ...
    const buttons = [
      [
        Markup.button.callback(TEXT.manageButtons.present,   `sm:${activeType}:set:${token}:present`),
        Markup.button.callback(TEXT.manageButtons.listening, `sm:${activeType}:set:${token}:listening`),
      ],
      [
        Markup.button.callback(TEXT.manageButtons.excused, `sm:${activeType}:set:${token}:excused`),
        Markup.button.callback(TEXT.manageButtons.absent,  `sm:${activeType}:set:${token}:absent`),
      ],
      ...(showCall ? [
        [
          Markup.button.callback(TEXT.manageButtons.markCalling,   `sm:${activeType}:call:${token}:responding`),
          Markup.button.callback(TEXT.manageButtons.markResponded, `sm:${activeType}:call:${token}:responded`),
        ],
        [Markup.button.callback(TEXT.manageButtons.markAway,    `sm:${activeType}:call:${token}:away`)],
        [Markup.button.callback(TEXT.manageButtons.clearCalled, `sm:${activeType}:call:${token}:clear`)],
      ] : []),
    ];

    if (session.type === 'personalRecitation' || session.type === 'groupRecitation') {
      buttons.push([Markup.button.callback(TEXT.manageButtons.editPage, `sm:${activeType}:editpage:${token}`)]);
    } else if (session.type === 'registeredSecondary') {
      buttons.push([Markup.button.callback(TEXT.manageButtons.editVerse, `sm:${activeType}:editverse:${token}`)]);
    }
    buttons.push([Markup.button.callback(TEXT.manageButtons.back, `sm:${activeType}:back:${token}`)]);
...
export function register(bot, storage) {
  ...
  bot.action(/^sm:(?:([a-zA-Z]+):)?pick:([ug]?\d+)$/, h.pick);
  bot.action(/^sm:(?:([a-zA-Z]+):)?set:([ug]?\d+):(present|listening|excused|absent)$/, h.setStatus);
  bot.action(/^sm:(?:([a-zA-Z]+):)?call:([ug]?\d+):(responding|responded|away|clear)$/, h.setCall);
  bot.action(/^sm:(?:([a-zA-Z]+):)?editpage:([ug]?\d+)$/, h.editPage);
  bot.action(/^sm:(?:([a-zA-Z]+):)?editverse:([ug]?\d+)$/, h.editVerse);
  bot.action(/^sm:(?:([a-zA-Z]+):)?back:([ug]?\d+)$/, h.backToIndex);
