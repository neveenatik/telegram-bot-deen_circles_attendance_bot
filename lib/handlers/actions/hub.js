...
export function groupTeachersView(groupId, teachers) {
  ...
  for (const type of GROUP_TEACHER_TYPES) {
    ...
    rows.push([Markup.button.callback(label, `mg:tcat:${groupId}:${type}`)]);
  }
  const untyped = list.filter((t) => !Array.isArray(t.types) || !t.types.some((r) => GROUP_TEACHER_TYPES.includes(r)));
  if (untyped.length) {
    rows.push([Markup.button.callback(`${OT.teacherCategoryOther} (${untyped.length})`, `mg:tcat:${groupId}:none`)]);
  }
  rows.push([Markup.button.callback(OT.addTeacherButton, `mg:tadd:${groupId}`)]);
...
export function groupTeacherCategoryView(groupId, type, teachers) {
  const list = Array.isArray(teachers) ? teachers : [];
  const inCat = type === 'none'
    ? list.filter((t) => !Array.isArray(t.types) || !t.types.some((r) => GROUP_TEACHER_TYPES.includes(r)))
  ...
  const rows = inCat.map((t) => [Markup.button.callback(clampButtonLabel(t.name), `mg:tch:${groupId}:${t.userId}`)]);
  rows.push([Markup.button.callback(HUB.backButton, `mg:teach:${groupId}`)]);
  rows.push([Markup.button.callback(TEXT.closeButton, 'msg:dismiss')]);
  const hint = inCat.length ? OT.teacherCategoryHint : OT.teacherCategoryEmpty;
  return { text: `${OT.teacherCategoryTitle(label)}\n\n${hint}`, keyboard: Markup.inlineKeyboard(rows) };
}

// Add is role-first: pick one role, then paste a list of `id | name` lines. Every
// pasted teacher is added with that role; extra roles can be toggled afterwards.
export function groupAddTeacherRoleView(groupId) {
  const rows = GROUP_TEACHER_TYPES.map((type) => [
    Markup.button.callback(TEXT.teacherTypeLabel[type] || type, `mg:taddr:${groupId}:${type}`),
  ]);
  rows.push([Markup.button.callback(TEXT.backButton, `mg:teach:${groupId}`)]);
  rows.push([Markup.button.callback(TEXT.closeButton, 'msg:dismiss')]);
  return { text: HUB.addTeacherPickRoleTitle, keyboard: Markup.inlineKeyboard(rows) };
}
...
export function createHandlers({ storage, telegram }) {
  const { getMaster, getPendingRegistrations, getAllSessions, getCurrentSeries, getTeachers, saveTeachers, getTrainingGroups, saveTrainingGroups, setParentGroup, setReplyPrompt, getClassTimezone } = storage;
  ...
  async function openPending(ctx) {
    ...
    await ctx.editMessageText(pendingStudentsText(pending, 0, tz), {
      parse_mode: 'Markdown',
  ...
  async function teacherCategory(ctx) {
    ...
    if (type !== 'none' && !GROUP_TEACHER_TYPES.includes(type)) return ctx.answerCbQuery(OT.teacherNotFound);
    const list = await getTeachers(groupId);
    const view = groupTeacherCategoryView(groupId, type, list);
    await ctx.editMessageText(view.text, { parse_mode: 'Markdown', ...view.keyboard });
    await ctx.answerCbQuery();
  }
  ...
  async function setTeacherType(ctx) {
    ...
    teacher.types = GROUP_TEACHER_TYPES.filter((t) => current.has(t));
    await saveTeachers(groupId, list);
    const view = groupTeacherTypeView(groupId, teacher);
    await ctx.editMessageText(view.text, { parse_mode: 'Markdown', ...view.keyboard });
    await ctx.answerCbQuery(OT.teacherTypeChanged(TEXT.teacherTypesLabel(teacher.types)));
  }
  ...
  async function addTeacherPrompt(ctx) {
    if (!await ensureAdmin(ctx)) return ctx.answerCbQuery(TEXT.adminOnly);
    const groupId = ctx.match[1];
    const view = groupAddTeacherRoleView(groupId);
    await ctx.editMessageText(view.text, { parse_mode: 'Markdown', ...view.keyboard });
    await ctx.answerCbQuery();
  }

  // Role chosen; now prompt for a plain list of `id | name` lines to add with it.
  async function addTeacherNamesPrompt(ctx) {
    if (!await ensureAdmin(ctx)) return ctx.answerCbQuery(TEXT.adminOnly);
    const groupId = ctx.match[1];
    const role = ctx.match[2];
    if (!GROUP_TEACHER_TYPES.includes(role)) return ctx.answerCbQuery(OT.teacherNotFound);
    await beginForceReplyAwaiting(ctx, {
      setReplyPrompt,
      groupId,
      record: { action: 'groupAddTeacher', role },
      sendPrompt: () => ctx.reply(
        HUB.addTeacherNamesPrompt(TEXT.teacherTypeLabel[role] || role),
  ...
  return {
    ...
    openTeachers, teacherCategory, teacherMenu, teacherTypeMenu, setTeacherType,
    removeTeacherConfirmView, removeTeacher, addTeacherPrompt, addTeacherNamesPrompt, renameTeacherPrompt,
...
export function register(bot, storage) {
  ...
  bot.action(/^mg:tcat:(-?\d+):([a-zA-Z]+)$/, h.teacherCategory);
  bot.action(/^mg:tadd:(-?\d+)$/, h.addTeacherPrompt);
  bot.action(/^mg:taddr:(-?\d+):([a-zA-Z]+)$/, h.addTeacherNamesPrompt);
