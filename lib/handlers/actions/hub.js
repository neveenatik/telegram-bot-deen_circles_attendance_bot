...
export function groupTeachersView(groupId, teachers) {
  ...
  for (const type of GROUP_TEACHER_TYPES) {
    ...
    rows.push([Markup.button.callback(TEXT.teacherTypeLabel[type] || type, 'mg:noop')]);
    for (const t of inType) {
      seen.add(String(t.userId));
      rows.push([Markup.button.callback(clampButtonLabel(t.name), `mg:tch:${groupId}:${t.userId}`)]);
    }
  }
  // Any teacher with no recognized role still shows, under an "other" section.
  const untyped = list.filter((t) => !seen.has(String(t.userId)));
  if (untyped.length) {
    rows.push([Markup.button.callback(OT.teacherSectionOther, 'mg:noop')]);
...
export function createHandlers({ storage, telegram }) {
  ...
  // A non-actionable button (e.g. a role section header) just clears the spinner.
