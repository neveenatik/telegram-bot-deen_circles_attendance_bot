...
export function createHandlers({
  ...
  async function recite(ctx, attendedMain = true) {
    const groupId = groupIdFromCtx(ctx);
    const activeSession = await getActiveSession(groupId);
    const activeType = activeSession?.type || null;
    const session = activeSession?.session || null;
    ...
    if (!member) {
      ...
      participants.register(session, name, { status, memberId: uid, registeredAt: Date.now(), attendedMain });
      await saveParticipant(groupId, activeType, session, name);
      await refreshSessionWidget(telegram, session, master, async () => saveSession(groupId, activeType, session));
      return ctx.answerCbQuery(attendedMain ? TEXT.reciteAttestationAlert : TEXT.reciteAttestationNoMainAlert, { show_alert: true });
    }
...
export function register(bot, storage) {
  ...
  bot.action('a:recite', (ctx) => handlers.recite(ctx, true));
  bot.action('a:recite:nomain', (ctx) => handlers.recite(ctx, false));
}
