...
export function createHandlers({
  ...
  const { getMaster, saveMaster, getActiveSession, saveSession, saveParticipant, getPageProgress, getParentGroupId, getPendingRegistrations, savePendingRegistrations } = storage;
  ...
  async function mark(ctx) {
    ...
    if (!member) {
      ...
      // Training lists (groups linked to a main group) do NOT add walk-ins to the
      // roster directly — an accidental tap shouldn't pollute the members list.
      // Instead, queue them as a pending student for teacher approval; approving
      // adds them to this training group AND backfills them into the main group.
      // They are still counted present in this live session below.
