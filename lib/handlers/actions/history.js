...
function sessionEditorKb(groupId, series, recordIndex, session, page = 0, pageSize = 8) {
  ...
  rows.push([Markup.button.callback(TEXT.historyReportButton, `h:report:${groupId}:${series}:${recordIndex}`)]);
  rows.push([
...
export function createHandlers({ storage, telegram }) {
  ...
  async function setStatus(ctx) {
    ...
    const { names: refreshedNames, safePage, totalPages, keyboard } = sessionEditorKb(groupId, series, recordIndex, refreshedPicked, page);
    await ctx.editMessageText(
  ...
  return { home, seriesReport, sessionReport, edit, editType, session, pick, editVerse, setStatus, noop };
}
