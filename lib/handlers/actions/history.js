...
// attendance status for every type and, for recitation-correction sessions,
// also the recited verse. Shared by the `pick` action and the text-input reply
// handler (text.js) so a verse edit refreshes the same panel in place.
export function renderHistoryMemberMenu(groupId, series, recordIndex, session, token) {
  ...
  if (session.type === 'registeredSecondary') {
    lines.push(TEXT.historyMemberVerseLine(participants.getVerse(session, name)));
  }
  ...
  if (session.type === 'registeredSecondary') {
    rows.push([Markup.button.callback(TEXT.manageButtons.editVerse, `h:everse:${groupId}:${series}:${recordIndex}:${token}`)]);
  }
...
export function createHandlers({ storage, telegram }) {
  ...
  // awaiting flow (resolved in text.js → historyEditVerse).
  ...
  return { home, seriesReport, sessionReport, edit, editType, session, pick, editVerse, editTitle, setStatus, noop };
}
...
export function register(bot, storage) {
  ...
  bot.action(/^h:everse:(-?\d+):(\d+):(\d+):([ug]?\d+)$/, h.editVerse);
