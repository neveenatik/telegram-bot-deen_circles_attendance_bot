...
export function createHandlers({ storage, telegram, ns = 'h', resolveContext, orderNames, backBuilders } = {}) {
  const { getAllSessions, getSessions, saveSessions, deleteSession, getSessionParticipants, getMaster, setReplyPrompt, getPendingRegistrations, getSessionTeacher } = storage;
  ...
  async function setStatus(ctx) {
    ...
    const editor = renderHistorySessionEditor(groupId, series, recordIndex, refreshedPicked, page, { ...nav(gref), caps: rc.caps });
    await ctx.editMessageText(editor.text, { parse_mode: 'Markdown', ...editor.keyboard });
