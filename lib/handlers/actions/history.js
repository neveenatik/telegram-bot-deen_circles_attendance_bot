...
export function register(bot, storage) {
  ...
  bot.action(/^h:home:(\d+)$/, h.home);
  bot.action(/^h:rep:(\d+)$/, h.seriesReport);
  bot.action(/^h:report:(\d+):(\d+)$/, h.sessionReport);
  bot.action(/^h:edit:(\d+):(\d+)$/, h.edit);
  bot.action(/^h:session:(\d+):(\d+):(\d+)$/, h.session);
  bot.action(/^h:pick:(\d+):(\d+):(\d+)$/, h.pick);
  bot.action(/^h:set:(\d+):(\d+):(\d+):(present|listening|excused|absent|pending)$/, h.setStatus);
