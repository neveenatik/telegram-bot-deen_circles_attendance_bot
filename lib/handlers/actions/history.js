...
import { sortArabic, replyChunked, ensureNoPendingAwaiting } from '../../helpers.js';
...
export function historyHomeKb(groupId, series) {
  return Markup.inlineKeyboard([
    [Markup.button.callback(TEXT.historyShowReportButton, `h:rep:${groupId}:${series}`)],
    [Markup.button.callback(TEXT.historyEditSessionsButton, `h:edit:${groupId}:${series}`)],
...
function editTypeMenuKb(scoped, groupId, series) {
  const rows = [];
  for (const type of ACTIVE_SESSION_TYPES) {
  ...
  if (otherCount) {
    rows.push([Markup.button.callback(
      ...
      `h:etype:${groupId}:${series}:other:0`
    )]);
  }
  rows.push([
    Markup.button.callback(TEXT.backButton, `h:home:${groupId}:${series}`),
    Markup.button.callback(TEXT.closeButton, 'msg:dismiss'),
  ]);
  return Markup.inlineKeyboard(rows);
}

function editSessionKb(scoped, groupId, series, typeKey, page = 0, pageSize = 8) {
  ...
  const rows = entries.slice(start, start + pageSize).map(({ session: s, absoluteIndex }) => {
    ...
    return [Markup.button.callback(label, `h:session:${groupId}:${series}:${absoluteIndex}:0`)];
  });

  if (totalPages > 1) {
    rows.push([
      ...(safePage > 0 ? [Markup.button.callback(TEXT.navigationPrevButton, `h:etype:${groupId}:${series}:${typeKey}:${safePage - 1}`)] : []),
      Markup.button.callback(`📄 ${safePage + 1}/${totalPages}`, 'h:noop'),
      ...(safePage < totalPages - 1 ? [Markup.button.callback(TEXT.navigationNextButton, `h:etype:${groupId}:${series}:${typeKey}:${safePage + 1}`)] : []),
    ]);
  }

  rows.push([
    Markup.button.callback(TEXT.backButton, `h:edit:${groupId}:${series}`),
    Markup.button.callback(TEXT.closeButton, 'msg:dismiss'),
  ]);
...
function sessionEditorKb(groupId, series, recordIndex, session, page = 0, pageSize = 8) {
  ...
  const rows = names.slice(start, start + pageSize).map((name, i) => {
    ...
    const status = participants.getStatus(session, name);
    return [Markup.button.callback(clampButtonLabel(`${st(status).e} ${name}`), `h:pick:${groupId}:${series}:${recordIndex}:${token}`)];
  });

  if (totalPages > 1) {
    rows.push([
      ...(safePage > 0 ? [Markup.button.callback(TEXT.navigationPrevButton, `h:session:${groupId}:${series}:${recordIndex}:${safePage - 1}`)] : []),
      Markup.button.callback(`📄 ${safePage + 1}/${totalPages}`, 'h:noop'),
      ...(safePage < totalPages - 1 ? [Markup.button.callback(TEXT.navigationNextButton, `h:session:${groupId}:${series}:${recordIndex}:${safePage + 1}`)] : []),
    ]);
  }

  rows.push([
    Markup.button.callback(TEXT.historyReportButton, `h:report:${groupId}:${series}:${recordIndex}`),
    Markup.button.callback(TEXT.historyEditTitleButton, `h:etitle:${groupId}:${series}:${recordIndex}:${safePage}`),
  ]);
  ...
  if (session.type === 'registeredSecondary') {
    rows.push([Markup.button.callback(TEXT.historyEditVersesButton, `h:vlist:${groupId}:${series}:${recordIndex}:0`)]);
  }
  rows.push([
    Markup.button.callback(TEXT.historyBackToSessionsButton, `h:etype:${groupId}:${series}:${typeKeyOf(session)}:0`),
    Markup.button.callback(TEXT.historyBackToHomeButton, `h:home:${groupId}:${series}`),
  ]);
...
export function renderHistorySessionEditor(groupId, series, recordIndex, session, page = 0) {
  const { names, safePage, totalPages, keyboard } = sessionEditorKb(groupId, series, recordIndex, session, page);
  return {
...
export function renderHistoryMemberMenu(groupId, series, recordIndex, session, token) {
  ...
  const setCb = (status) => `h:set:${groupId}:${series}:${recordIndex}:${token}:${status}`;
  const rows = [
    [
  ...
  if (session.type === 'registeredSecondary') {
    ...
    const flagCb = (flag, value) => `h:rflag:${groupId}:${series}:${recordIndex}:${token}:${flag}:${value}`;
  ...
  rows.push([Markup.button.callback(TEXT.backButton, `h:session:${groupId}:${series}:${recordIndex}:${memberPage}`)]);
...
// them back correctly.
export function renderHistoryVerseList(groupId, series, recordIndex, session, page = 0, pageSize = 8) {
  ...
  const rows = slice.map(({ name, fullIndex, status }) => {
    ...
    return [Markup.button.callback(label, `h:everse:${groupId}:${series}:${recordIndex}:${token}:v${safePage}`)];
  });

  if (totalPages > 1) {
    rows.push([
      ...(safePage > 0 ? [Markup.button.callback(TEXT.navigationPrevButton, `h:vlist:${groupId}:${series}:${recordIndex}:${safePage - 1}`)] : []),
      Markup.button.callback(`📄 ${safePage + 1}/${totalPages}`, 'h:noop'),
      ...(safePage < totalPages - 1 ? [Markup.button.callback(TEXT.navigationNextButton, `h:vlist:${groupId}:${series}:${recordIndex}:${safePage + 1}`)] : []),
    ]);
  }

  rows.push([
    Markup.button.callback(TEXT.backButton, `h:session:${groupId}:${series}:${recordIndex}:0`),
    Markup.button.callback(TEXT.closeButton, 'msg:dismiss'),
  ]);
...
export function createHandlers({ storage, telegram }) {
  const { getAllSessions, getSessions, saveSessions, getMaster, setAwaiting, getAwaiting } = storage;

  // The class-history panel is delivered to the admin's DM, so `isAdmin(ctx)`
  // (which is false in private chats) can't be used. Every h:* callback embeds
  // the target group id at match[1]; verify admin membership against that group.
  ...
  async function edit(ctx) {
    ...
    await ctx.editMessageText(TEXT.historyEditTypesText(series, scoped.length), {
      parse_mode: 'Markdown',
      ...editTypeMenuKb(scoped, groupId, series),
    });
    await ctx.answerCbQuery(TEXT.refreshed);
  }
  ...
  async function pick(ctx) {
    ...
    const menu = renderHistoryMemberMenu(groupId, series, recordIndex, picked, token);
  ...
  async function editVerse(ctx) {
    ...
    const base = {
      action: 'historyEditVerse', groupId, chatId: ctx.chat.id, msgId,
    ...
    await setAwaiting(awaitKey, uid, { ...base, promptMsgId: null, awaitingPrompt: true });
    ...
    await setAwaiting(awaitKey, uid, { ...base, promptMsgId: prompt.message_id, awaitingPrompt: false });
  }
  ...
  async function editTitle(ctx) {
    ...
    const msgId = ctx.callbackQuery.message.message_id;
    const base = {
      action: 'historyEditTitle', groupId, chatId: ctx.chat.id, msgId,
    ...
    await setAwaiting(awaitKey, uid, { ...base, promptMsgId: null, awaitingPrompt: true });
    const prompt = await ctx.reply(TEXT.historyEditTitlePrompt(picked.name), {
      parse_mode: 'Markdown',
  ...
  async function setStatus(ctx) {
    ...
    const editor = renderHistorySessionEditor(groupId, series, recordIndex, refreshedPicked, page);
    await ctx.editMessageText(editor.text, { parse_mode: 'Markdown', ...editor.keyboard });
  ...
  async function setReciteFlag(ctx) {
    ...
    const menu = renderHistoryMemberMenu(groupId, series, recordIndex, refreshedPicked, token);
    if (!menu) return ctx.answerCbQuery(TEXT.memberNotFound);
    await ctx.editMessageText(menu.text, { parse_mode: 'Markdown', ...menu.keyboard });
...
export function register(bot, storage) {
  ...
  bot.action(/^h:home:(-?\d+):(\d+)$/, h.home);
  bot.action(/^h:rep:(-?\d+):(\d+)$/, h.seriesReport);
  bot.action(/^h:report:(-?\d+):(\d+):(\d+)$/, h.sessionReport);
  bot.action(/^h:edit:(-?\d+):(\d+)$/, h.edit);
  bot.action(/^h:etype:(-?\d+):(\d+):([a-zA-Z]+):(\d+)$/, h.editType);
  bot.action(/^h:session:(-?\d+):(\d+):(\d+):(\d+)$/, h.session);
  bot.action(/^h:pick:(-?\d+):(\d+):(\d+):([ug]?\d+)$/, h.pick);
  bot.action(/^h:vlist:(-?\d+):(\d+):(\d+):(\d+)$/, h.verseList);
  bot.action(/^h:everse:(-?\d+):(\d+):(\d+):([ug]?\d+):v(\d+)$/, h.editVerse);
  bot.action(/^h:etitle:(-?\d+):(\d+):(\d+):(\d+)$/, h.editTitle);
  bot.action(/^h:set:(-?\d+):(\d+):(\d+):([ug]?\d+):(present|listening|excused|absent|pending)$/, h.setStatus);
  bot.action(/^h:rflag:(-?\d+):(\d+):(\d+):([ug]?\d+):(main|backup):(0|1)$/, h.setReciteFlag);
  bot.action('h:noop', h.noop);
}
