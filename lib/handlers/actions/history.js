...
function historyHomeKb(groupId, series) {
  return Markup.inlineKeyboard([
    ...
    [Markup.button.callback(TEXT.historyEditSessionsButton, `h:edit:${groupId}:${series}:0`)],
...
function buildClassHistoryReport(scoped) {
  ...
  const typeOrder = ['main', 'training', 'open', 'registeredSecondary', 'personalRecitation', 'groupRecitation'];
  const typeTitle = {
...
function editSessionKb(scoped, groupId, series, page = 0, pageSize = 8) {
  ...
  if (totalPages > 1) {
    rows.push([
      ...(safePage > 0 ? [Markup.button.callback(TEXT.navigationPrevButton, `h:edit:${groupId}:${series}:${safePage - 1}`)] : []),
      Markup.button.callback(`📄 ${safePage + 1}/${totalPages}`, 'h:noop'),
      ...(safePage < totalPages - 1 ? [Markup.button.callback(TEXT.navigationNextButton, `h:edit:${groupId}:${series}:${safePage + 1}`)] : []),
    ]);
  }
...
function sessionEditorKb(groupId, series, recordIndex, session, page = 0, pageSize = 8) {
  ...
  const rows = names.slice(start, start + pageSize).map((name, i) => {
    ...
    const status = participants.getStatus(session, name);
    return [Markup.button.callback(clampButtonLabel(`${st(status).e} ${name}`), `h:pick:${groupId}:${series}:${recordIndex}:${memberIndex}`)];
  });
  ...
  rows.push([
    Markup.button.callback(TEXT.historyBackToSessionsButton, `h:edit:${groupId}:${series}:0`),
...
export function createHandlers({ storage }) {
  ...
  async function edit(ctx) {
    ...
    const { safePage, totalPages, keyboard } = editSessionKb(scoped, groupId, series, Number.isInteger(requestedPage) ? requestedPage : 0);
  ...
  async function pick(ctx) {
    ...
    const series = parseInt(ctx.match[2], 10);
    const recordIndex = parseInt(ctx.match[3], 10);
    const memberIndex = parseInt(ctx.match[4], 10);
    ...
    const kb = Markup.inlineKeyboard([
      [
        Markup.button.callback(TEXT.historyStatusButtons.present, `h:set:${groupId}:${series}:${recordIndex}:${memberIndex}:present`),
        Markup.button.callback(TEXT.historyStatusButtons.listening, `h:set:${groupId}:${series}:${recordIndex}:${memberIndex}:listening`),
      ],
      [
        Markup.button.callback(TEXT.historyStatusButtons.excused, `h:set:${groupId}:${series}:${recordIndex}:${memberIndex}:excused`),
        Markup.button.callback(TEXT.historyStatusButtons.absent, `h:set:${groupId}:${series}:${recordIndex}:${memberIndex}:absent`),
      ],
      [Markup.button.callback(TEXT.historyStatusButtons.pending, `h:set:${groupId}:${series}:${recordIndex}:${memberIndex}:pending`)],
  ...
  return { home, seriesReport, sessionReport, edit, session, pick, setStatus, noop };
}
...
export function register(bot, storage) {
  ...
  bot.action(/^h:edit:(-?\d+):(\d+):(\d+)$/, h.edit);
  bot.action(/^h:session:(-?\d+):(\d+):(\d+):(\d+)$/, h.session);
  bot.action(/^h:pick:(-?\d+):(\d+):(\d+):(\d+)$/, h.pick);
  bot.action(/^h:set:(-?\d+):(\d+):(\d+):(\d+):(present|listening|excused|absent|pending)$/, h.setStatus);
