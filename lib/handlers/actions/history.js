...
import { sortArabic, replyChunked } from '../../helpers.js';
...
export function createHandlers({ storage }) {
  ...
  async function pick(ctx) {
    ...
    const status = participants.getStatus(picked, name);
    const memberPage = Math.max(0, Math.floor(index / 8));

    const kb = Markup.inlineKeyboard([
      [
        Markup.button.callback(TEXT.historyStatusButtons.present, `h:set:${groupId}:${series}:${recordIndex}:${token}:present`),
        Markup.button.callback(TEXT.historyStatusButtons.listening, `h:set:${groupId}:${series}:${recordIndex}:${token}:listening`),
      ],
      [
        Markup.button.callback(TEXT.historyStatusButtons.excused, `h:set:${groupId}:${series}:${recordIndex}:${token}:excused`),
        Markup.button.callback(TEXT.historyStatusButtons.absent, `h:set:${groupId}:${series}:${recordIndex}:${token}:absent`),
      ],
      [Markup.button.callback(TEXT.historyStatusButtons.pending, `h:set:${groupId}:${series}:${recordIndex}:${token}:pending`)],
      [Markup.button.callback(TEXT.backButton, `h:session:${groupId}:${series}:${recordIndex}:${memberPage}`)],
    ]);
  ...
  return { home, seriesReport, sessionReport, edit, editType, session, pick, setStatus, noop };
}
