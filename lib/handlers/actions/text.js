...
import { renderClassHome, renderRoster, renderTeachers, renderManagersMenu } from './offline.js';
...
export function createHandlers({ storage, telegram }) {
  const {
    ...
    addClassManager, listClassManagers,
  } = storage;
  ...
  async function onText(ctx, next) {
    ...
    if (pending.action === 'offlineAddManager') {
      ...
      const role = pending.role === 'assistant' ? 'assistant' : 'operator';
      const res = await addClassManager(cls.rowId, uid, role, ctx.from.id);
      ...
      return replyEphemeral(ctx, TEXT.offline.managerAdded(`#${uid}`, TEXT.offline.roleLabel(role)), { parse_mode: 'Markdown' });
    }
