import { groupIdFromCtx, formatPages, parsePageInput, replyEphemeral, splitEntries, logTelegramError } from '../../helpers.js';
...
export function register(bot, storage) {
  const {
    ...
    getPendingRegistrations, savePendingRegistrations,
  } = storage;
    ...
    const kbRows = slice.map((row) => (
      String(row.userId) === String(confirmDismissUserId)
        ...
        : [
            ...
            { text: '✏️ تعديل الاسم', callback_data: `pr:edit:${row.userId}:${safePage}` },
  ...
  bot.on('text', async (ctx) => {
    ...
    if (pending.action === 'editSessionName') {
      ...
      bot.telegram.editMessageText(
        ...
        { parse_mode: 'Markdown', ...manageKb(session, master, page) }
