import { formatPages, parsePageInput, replyEphemeral, splitEntries, logTelegramError, escapeTelegramMarkdown } from '../../helpers.js';
...
export function createHandlers({ storage, telegram }) {
  ...
  async function onText(ctx, next) {
    ...
    if (pending.action === 'historyEditVerse') {
      ...
      const view = renderHistoryVerseList(groupId, series, recordIndex, typeSessions[idx], pending.verseListPage || 0);
      telegram.editMessageText(
        pending.chatId, pending.msgId, undefined,
        view.text,
    ...
    if (pending.action === 'historyEditTitle') {
      ...
      const editor = renderHistorySessionEditor(groupId, series, recordIndex, typeSessions[idx], pending.memberPage || 0);
      telegram.editMessageText(
        pending.chatId, pending.msgId, undefined,
        editor.text,
