...
import { groupIdFromCtx, sortArabic, beginForceReplyAwaiting, getDisplayName, escapeTelegramMarkdown } from '../../helpers.js';
...
export function pendingStudentsText(rows, page = 0) {
...
export function createHandlers({ storage, telegram, refreshSessionWidget = defaultRefreshSessionWidget }) {
  const { getMaster, saveMaster, getActiveSession, getSession, saveSession, setReplyPrompt, getPendingRegistrations, savePendingRegistrations, getTeachers, saveTeachers, getTrainingGroups, getParentGroupId, addMembers } = storage;
