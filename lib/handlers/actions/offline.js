...
export function renderTeachers(cls, teachers) {
  ...
  // Group teachers under a header per role (a teacher appears under each role
  // she holds) so the list reads by section instead of one long flat list.
  const seen = new Set();
  for (const type of TEACHER_TYPES) {
    const inType = list.filter((t) => Array.isArray(t.types) && t.types.includes(type));
    if (!inType.length) continue;
    rows.push([Markup.button.callback(TEXT.teacherTypeLabel[type] || type, 'o:noop')]);
    for (const t of inType) {
      seen.add(String(t.id));
      rows.push([Markup.button.callback(clampButtonLabel(t.name), `o:tch:${g}:${t.id}`)]);
    }
  }
  // Any teacher with no recognized role still shows, under an "other" section.
  const untyped = list.filter((t) => !seen.has(String(t.id)));
  if (untyped.length) {
    rows.push([Markup.button.callback(OT.teacherSectionOther, 'o:noop')]);
    for (const t of untyped) {
      rows.push([Markup.button.callback(clampButtonLabel(t.name), `o:tch:${g}:${t.id}`)]);
    }
  }
  rows.push([Markup.button.callback(TEXT.backButton, `o:cls:${g}`), ...dismissRow()]);
