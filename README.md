...
| `/pendingstudents` | عرض طلبات `/myid` المعلّقة وإضافة الطالبات مباشرة (للمشرف) |
...
1. المشرف يطلب من العضو تنفيذ `/myid` داخل المجموعة.
2. يحفظ البوت الطلب تلقائياً ضمن الطلبات المعلّقة للمجموعة.
3. المشرف يراجع الطلبات من `/pendingstudents` ثم يضيف الطالبة مباشرة بزر واحد، أو يضيفها يدوياً عبر `/addstudent 123456789 | أحمد محمد`.
3. المشرف يبدأ قائمة: `/startlist اجتماع يونيو`
4. الأعضاء يسجّلون حضورهم من واجهة الأزرار: حاضر / مستمع / غير قادر.
5. يمكن للمشرف أثناء المجلس إرسال `/lessonstart` ثم `/lessonreminder` لتجميع تأكيدات المتابعة بنقاط محفوظة داخل الجلسة.
6. المشرف يراجع الحالات من `/status` أو من واجهة `/editlist`.
7. المشرف ينهي القائمة: `/stoplist` ← يتم تعطيل الواجهة وتوليد التقرير النهائي مع ملخص تأكيدات المتابعة وكفارة المجلس
