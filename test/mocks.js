...
export function makeStorage(overrides = {}) {
  return {
    ...
    renameOfflineTeacher: async () => ({ ok: true, name: 'معلمة' }),
    setOfflineTeacherTypes: async () => ({ ok: true, name: 'معلمة', types: ['courseteacher'] }),
    removeOfflineTeacher: async () => ({ ok: true, name: 'معلمة' }),
    getOfflineTrainingGroups: async () => [],
    addOfflineTrainingGroup: async (_g, name) => ({ ok: true, group: { id: 'tg-1', name } }),
    renameOfflineTrainingGroup: async (_g, _id, name) => ({ ok: true, name }),
