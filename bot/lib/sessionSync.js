export function syncSessionNamesFromMaster(session, master) {
  ...
  // Recitation-correction lists (registeredSecondary) are self-registered as well:
  // they start empty and fill only from student taps, so master must never seed
  // them with the full roster — otherwise a refresh populates every member.
  if (session.allowPublicRegistration || session.type === 'registeredSecondary') {
  ...
  for (const member of masterMembers) {
    ...
    if (member.listNumber != null) rec.listNumber = member.listNumber;
    if (src) {
