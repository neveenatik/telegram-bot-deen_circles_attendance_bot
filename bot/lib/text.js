...
// The ⏳ tag appended to a walk-in still awaiting admin approval. Shared by the
// end-of-session report (via pendingSuffix, matched against the pending queue)
// and the live list widget (via the participant's pendingApproval flag) so the
// wording never drifts between the two surfaces.
const PENDING_APPROVAL_TAG = '⏳ قيد الموافقة';

function pendingSuffix(name, opts) {
  return opts?.isPending && opts.isPending(name) ? ` ${PENDING_APPROVAL_TAG}` : '';
}
...
export const TEXT = {
  ...
  // Lightweight, data-free ping posted in the group while a session is active
  // and there are pending join requests. Tapping the button opens the full
  // pending-registrations panel privately (in the admin's DM).
  pendingNudgeGroup: (count) => `🔔 *طلبات انضمام بانتظار الاعتماد: ${count}*\n_اضغطي الزر لفتح لوحة الطلبات في محادثتنا الخاصة._`,
  pendingNudgeButton: '📝 فتح طلبات التسجيل',
  // Tag appended to a walk-in's name in the live list while she awaits approval.
  pendingApprovalTag: PENDING_APPROVAL_TAG,
...
`.trim(),
  ...
  offline: {
    ...
    deleteClassButton: '🗑️ حذف الصف',
    deleteClassConfirm: (name) => `⚠️ هل تريدين حذف الصف *${name}* نهائياً؟\n(سيتم حذف الطالبات والمعلمات والجلسات والمواد المرتبطة به ولا يمكن التراجع)`,
    confirmDeleteClassButton: '🗑️ نعم، احذفي الصف',
    classDeleted: (name) => `✅ تم حذف الصف *${name}* بنجاح.`,
    classDeletedToast: '✅ تم حذف الصف.',
    classDeleteFailed: '⚠️ تعذّر حذف الصف. حاولي لاحقاً.',
