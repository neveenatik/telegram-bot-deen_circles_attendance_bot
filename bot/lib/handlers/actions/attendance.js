...
export function createHandlers({
  ...
  async function mark(ctx) {
    ...
    if (!member) {
      ...
      // A queued walk-in awaits admin approval; tag her ⏳ in the live list until
      // she is approved (roster member) or dismissed.
      if (queuedPending) participants.setPendingApproval(session, name, true);
      ...
      await refreshSessionWidget(telegram, session, master, async () => saveSession(groupId, activeType, session));
      // A newly queued walk-in gets a data-free group ping so the admin can open
      // the private approval panel without leaving the live session.
      if (queuedPending) {
        await syncPendingNudge({ telegram, storage, groupId, session, type: activeType });
      }
      let toast;
  ...
  async function recite(ctx, attendedMain = true, backup = false) {
    ...
    if (!member) {
      ...
      await saveParticipant(groupId, activeType, session, name);
      await refreshSessionWidget(telegram, session, master, async () => saveSession(groupId, activeType, session));
      // Keep the live-session group nudge in sync with the new request.
      if (queuedPending) {
        await syncPendingNudge({ telegram, storage, groupId, session, type: activeType });
      }
