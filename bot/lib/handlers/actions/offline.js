...
// everywhere the shared session editor renders member/verse buttons. Exported so
// the force-reply refresh in text.js resolves the SAME positional tokens the
// editor handlers do — otherwise a refreshed list would encode alphabetical
// indices while taps resolve against list-number order (wrong student).
export const offlineOrderNames = (session) => participants.namesByListNumber(session);
// Offline classes can hold a session of any active type (attendance roll,
// training, open majlis, recitation-correction, and personal/group recitation).
// Uses SESSION_TYPES directly so new session types are supported offline
// automatically. The shared history editor (registerOfflineEditor) already
// renders/edits every one of these.
const TEACHER_TYPES = ['courseteacher', 'trainingteacher', 'recitationteacher', 'homeworkteacher'];
...
function fmtDate(iso, tz = 'Africa/Cairo') {
  return new Date(iso).toLocaleDateString('ar-EG', { timeZone: tz });
}
...
function capsFor(role) {
  ...
  return {
    ...
    manageMaterials: owner || operator, // upload/remove/resend teaching materials
...
export function renderClassHome(cls, role = 'owner') {
  ...
  rows.push([Markup.button.callback(TEXT.timetable.hubButton, `o:tt:${g}`)]);
  ...
  if (caps.manageMaterials) rows.push([Markup.button.callback(OT.materialsButton, `o:mat:${g}`)]);
  if (caps.manageMaterials) rows.push([Markup.button.callback(OT.homeworkButton, `o:hw:${g}`)]);
...
export function renderStudentMenu(cls, student, page = 0, trainingGroups = []) {
  ...
  const rows = [
    ...
    [Markup.button.callback(TEXT.homework.linkStudentButton, `o:slink:${g}:${ln}:${page}`)],
...
export function renderTeachers(cls, teachers) {
  ...
  // Level 1: one button per role category (each opens that role's list), plus a
  // safety-net "other" button for any teacher left without a recognized role.
  const rows = [];
  for (const type of TEACHER_TYPES) {
    const count = list.filter((t) => Array.isArray(t.types) && t.types.includes(type)).length;
    const label = `${TEXT.teacherCategoryLabel[type] || type} (${count})`;
    rows.push([Markup.button.callback(label, `o:tcat:${g}:${type}`)]);
  }
  const untyped = list.filter((t) => !Array.isArray(t.types) || !t.types.some((r) => TEACHER_TYPES.includes(r)));
  if (untyped.length) {
    rows.push([Markup.button.callback(`${OT.teacherCategoryOther} (${untyped.length})`, `o:tcat:${g}:none`)]);
  }
  rows.push([Markup.button.callback(OT.addTeacherButton, `o:addteach:${g}`)]);
  rows.push([Markup.button.callback(TEXT.backButton, `o:cls:${g}`), ...dismissRow()]);
  const hint = list.length ? OT.teachersManageHint : OT.teachersEmpty;
  return { text: `${OT.teachersTitle}\n\n${hint}`, keyboard: Markup.inlineKeyboard(rows) };
}

// Level 2: the teachers holding a given role (or the role-less ones for `none`).
export function renderTeacherCategory(cls, type, teachers) {
  const g = cls.rowId;
  const list = Array.isArray(teachers) ? teachers : [];
  const inCat = type === 'none'
    ? list.filter((t) => !Array.isArray(t.types) || !t.types.some((r) => TEACHER_TYPES.includes(r)))
    : list.filter((t) => Array.isArray(t.types) && t.types.includes(type));
  const label = type === 'none' ? OT.teacherCategoryOther : (TEXT.teacherCategoryLabel[type] || type);
  const rows = inCat.map((t) => [Markup.button.callback(clampButtonLabel(t.name), `o:tch:${g}:${t.id}`)]);
  rows.push([Markup.button.callback(TEXT.backButton, `o:teach:${g}`), ...dismissRow()]);
  const hint = inCat.length ? OT.teacherCategoryHint : OT.teacherCategoryEmpty;
  return { text: `${OT.teacherCategoryTitle(label)}\n\n${hint}`, keyboard: Markup.inlineKeyboard(rows) };
}

// Bulk add: first pick ONE role, then paste a plain list of names (one per
// line). Every pasted name is added with that role; extra roles can be toggled
// per teacher afterwards.
export function renderAddTeacherRoleMenu(cls) {
  const g = cls.rowId;
  const rows = TEACHER_TYPES.map((type) => [
    Markup.button.callback(TEXT.teacherTypeLabel[type] || type, `o:addteachr:${g}:${type}`),
  ]);
  rows.push([Markup.button.callback(TEXT.backButton, `o:teach:${g}`), ...dismissRow()]);
  return { text: OT.addTeacherPickRoleTitle, keyboard: Markup.inlineKeyboard(rows) };
}

// Per-teacher menu: rename, change assigned roles, or remove.
export function renderTeacherMenu(cls, teacher) {
  ...
  const typeLabel = TEXT.teacherTypesLabel(teacher.types);
  return { text: OT.teacherMenuTitle(teacher.name, typeLabel), keyboard: Markup.inlineKeyboard(rows) };
}

// Pick a teacher's roles. Each role toggles on/off; current roles are marked.
export function renderTeacherTypeMenu(cls, teacher) {
  const g = cls.rowId;
  const id = teacher.id;
  const current = new Set(teacher.types || []);
  const rows = TEACHER_TYPES.map((type) => {
    const label = TEXT.teacherTypeLabel[type] || type;
    const mark = current.has(type) ? '✅ ' : '';
...
function renderNewSessionMenu(cls) {
  const g = cls.rowId;
  const rows = SESSION_TYPES.map((type) => [
...
function renderSessionTypesMenu(cls, scoped, caps = capsFor('owner')) {
  ...
  if (caps.createSession) rows.push([Markup.button.callback(OT.newSessionButton, `o:newsess:${g}`)]);
  rows.push([Markup.button.callback(OT.allSessionsButton, `o:allsess:${g}:0`)]);
  rows.push([Markup.button.callback(TEXT.backButton, `o:cls:${g}`), ...dismissRow()]);
  return { text: `${OT.sessionsListTitle(cls.name)}\n\n${OT.pickSessionType}`, keyboard: Markup.inlineKeyboard(rows) };
}

function renderSessionsByType(cls, type, items, page = 0, caps = capsFor('owner'), tz = 'Africa/Cairo') {
  const g = cls.rowId;
  ...
  const rows = items.slice(start, start + SESSIONS_PAGE_SIZE).map(({ session, recordIndex }) => {
    const label = clampButtonLabel(`${recordIndex}. ${session.name} | ${fmtDate(session.endedAt || session.startedAt, tz)}`);
...
// Class-level roster view: every session across all types (series 1) in one
// paginated list. Each row keeps its absolute recordIndex so it opens the same
// session menu the per-type list does.
function renderAllSessions(cls, items, page = 0, caps = capsFor('owner'), tz = 'Africa/Cairo') {
  const g = cls.rowId;
  const totalPages = Math.max(1, Math.ceil(items.length / SESSIONS_PAGE_SIZE));
  const safePage = Math.max(0, Math.min(page, totalPages - 1));
  const start = safePage * SESSIONS_PAGE_SIZE;
  const rows = items.slice(start, start + SESSIONS_PAGE_SIZE).map(({ session, recordIndex }) => {
    const typeLabel = TEXT.historyTypeTitle[session.type] || session.type;
    const label = clampButtonLabel(
      OT.allSessionRow(recordIndex, typeLabel, session.name, fmtDate(session.endedAt || session.startedAt, tz)),
    );
    return [Markup.button.callback(label, `o:smenu:${g}:1:${recordIndex}`)];
  });
  if (totalPages > 1) {
    rows.push([
      ...(safePage > 0 ? [Markup.button.callback(TEXT.navigationPrevButton, `o:allsess:${g}:${safePage - 1}`)] : []),
      Markup.button.callback(`📄 ${safePage + 1}/${totalPages}`, 'o:noop'),
      ...(safePage < totalPages - 1 ? [Markup.button.callback(TEXT.navigationNextButton, `o:allsess:${g}:${safePage + 1}`)] : []),
    ]);
  }
  if (caps.createSession) rows.push([Markup.button.callback(OT.newSessionButton, `o:newsess:${g}`)]);
  rows.push([Markup.button.callback(TEXT.backButton, `o:sessions:${g}`), ...dismissRow()]);
  const body = items.length ? '' : `\n\n${OT.sessionsEmpty}`;
  return { text: `${OT.allSessionsTitle(cls.name)}${body}`, keyboard: Markup.inlineKeyboard(rows) };
}

function renderSessionMenu(cls, recordIndex, session, teacher, caps = capsFor('owner'), tz = 'Africa/Cairo') {
  const g = cls.rowId;
  const teacherLine = teacher
    ? `\n👩‍🏫 ${TEXT.teacherTypesLabel(teacher.types)} ${teacher.name}`
    : '';
  const text = `🗂️ *${session.name}*\n📅 ${fmtDate(session.endedAt || session.startedAt, tz)}${teacherLine}`;
  const rows = [
  ...
  if (caps.deleteSession) rows.push([Markup.button.callback(OT.deleteSessionButton, `o:sdel:${g}:1:${recordIndex}`)]);
  rows.push([Markup.button.callback(TEXT.backButton, `o:stype:${g}:${session.type}:0`), ...dismissRow()]);
  return { text, keyboard: Markup.inlineKeyboard(rows) };
}

// Confirm permanent deletion of one session.
function renderSessionDeleteConfirm(cls, recordIndex, session) {
  const g = cls.rowId;
  const rows = [
    [Markup.button.callback(OT.confirmDeleteSessionButton, `o:sdelx:${g}:1:${recordIndex}`)],
    [Markup.button.callback(TEXT.backButton, `o:smenu:${g}:1:${recordIndex}`), ...dismissRow()],
  ];
  return { text: OT.deleteSessionConfirm(session.name), keyboard: Markup.inlineKeyboard(rows) };
}
...
export function createHandlers({ storage, telegram }) {
  ...
  // Reports/date labels for an offline class render in its configured timezone
  // (group_settings.timezone), falling back to Cairo.
  ...
  // Share a join deep link for a student: opening it links her Telegram account
  // to this roster slot so she receives her homework in DM (studentHomework.ts).
  async function studentLink(ctx) {
    const rc = await resolveClass(ctx);
    if (!rc.ok) return ctx.answerCbQuery(rc.reason);
    if (denied(ctx, rc, 'editRoster')) return undefined;
    const listNumber = ctx.match[2];
    ...
    if (!username) return ctx.answerCbQuery(TEXT.homework.linkStudentNoUsername);
    const link = `https://t.me/${username}?start=hw-${rc.cls.rowId}-${listNumber}`;
    await ctx.reply(TEXT.homework.linkStudentText(student.name, link), {
  ...
  async function teacherCategory(ctx) {
    const rc = await resolveClass(ctx);
    if (!rc.ok) return ctx.answerCbQuery(rc.reason);
    if (denied(ctx, rc, 'editTeachers')) return undefined;
    const type = ctx.match[2];
    if (type !== 'none' && !TEACHER_TYPES.includes(type)) return ctx.answerCbQuery(OT.teacherNotFound);
    const list = await getTeachers(rc.cls.groupId);
    const view = renderTeacherCategory(rc.cls, type, list);
    await ctx.editMessageText(view.text, { parse_mode: 'Markdown', ...view.keyboard });
    await ctx.answerCbQuery();
  }
  ...
  async function addTeacherPrompt(ctx) {
    ...
    if (!rc.ok) return ctx.answerCbQuery(rc.reason);
    if (denied(ctx, rc, 'editTeachers')) return undefined;
    const view = renderAddTeacherRoleMenu(rc.cls);
    await ctx.editMessageText(view.text, { parse_mode: 'Markdown', ...view.keyboard });
    await ctx.answerCbQuery();
  }

  // Role chosen; now prompt for a plain list of names to add with that role.
  async function addTeacherNamesPrompt(ctx) {
    const rc = await resolveClass(ctx);
    if (!rc.ok) return ctx.answerCbQuery(rc.reason);
    if (denied(ctx, rc, 'editTeachers')) return undefined;
    const role = ctx.match[2];
    if (!TEACHER_TYPES.includes(role)) return ctx.answerCbQuery(OT.teacherNotFound);
    await beginForceReplyAwaiting(ctx, {
      setReplyPrompt,
      groupId: rc.cls.groupId,
      record: { action: 'offlineAddTeacher', gref: rc.cls.rowId, role },
      sendPrompt: () => ctx.reply(
        OT.addTeacherNamesPrompt(TEXT.teacherTypeLabel[role] || role),
  ...
  async function setTeacherType(ctx) {
    ...
    if (!teacher) return ctx.answerCbQuery(OT.teacherNotFound);
    // Toggle this role on/off, keeping at least one role.
    const current = new Set(teacher.types || []);
    if (current.has(type)) {
      if (current.size === 1) return ctx.answerCbQuery(OT.teacherNeedsRole);
      current.delete(type);
    } else {
      current.add(type);
    }
    const nextTypes = TEACHER_TYPES.filter((t) => current.has(t));
    const res = await setOfflineTeacherTypes(rc.cls.groupId, teacherId, nextTypes);
    if (!res.ok) return ctx.answerCbQuery(OT.teacherNotFound);
    const updated = { ...teacher, types: res.types || nextTypes };
    const view = renderTeacherTypeMenu(rc.cls, updated);
    await ctx.editMessageText(view.text, { parse_mode: 'Markdown', ...view.keyboard });
    await ctx.answerCbQuery(OT.teacherTypeChanged(TEXT.teacherTypesLabel(updated.types)));
  }
  ...
  async function createSession(ctx) {
    ...
    if (denied(ctx, rc, 'createSession')) return undefined;
    const type = ctx.match[2];
    if (!SESSION_TYPES.includes(type)) return ctx.answerCbQuery(OT.notFound);
  ...
  async function sessionsByType(ctx) {
    ...
    const view = renderSessionsByType(rc.cls, type, items, page, rc.caps, await classTz(rc.cls.groupId));
    await ctx.editMessageText(view.text, { parse_mode: 'Markdown', ...view.keyboard });
    await ctx.answerCbQuery();
  }
  ...
  async function sessionMenu(ctx) {
    ...
    const view = renderSessionMenu(rc.cls, recordIndex, picked, teacher, rc.caps, await classTz(rc.cls.groupId));
    await ctx.editMessageText(view.text, { parse_mode: 'Markdown', ...view.keyboard });
    await ctx.answerCbQuery();
  }

  // Class-level roster: every session across all types (series 1), paginated.
  async function allSessions(ctx) {
    const rc = await resolveClass(ctx);
    if (!rc.ok) return ctx.answerCbQuery(rc.reason);
    const page = parseInt(ctx.match[2], 10) || 0;
    const all = await getAllSessions(rc.cls.groupId);
    const scoped = sessionsInSeries(all, 1);
    const items = scoped.map((session, i) => ({ session, recordIndex: i + 1 }));
    const view = renderAllSessions(rc.cls, items, page, rc.caps, await classTz(rc.cls.groupId));
    await ctx.editMessageText(view.text, { parse_mode: 'Markdown', ...view.keyboard });
    await ctx.answerCbQuery();
  }

  async function sessionDeleteConfirm(ctx) {
    const rc = await resolveClass(ctx);
    if (!rc.ok) return ctx.answerCbQuery(rc.reason);
    if (denied(ctx, rc, 'deleteSession')) return undefined;
    const recordIndex = parseInt(ctx.match[3], 10);
    const all = await getAllSessions(rc.cls.groupId);
    const scoped = sessionsInSeries(all, 1);
    const picked = scoped[recordIndex - 1];
    if (!picked) return ctx.answerCbQuery(TEXT.invalidRecordIndex);
    const view = renderSessionDeleteConfirm(rc.cls, recordIndex, picked);
    await ctx.editMessageText(view.text, { parse_mode: 'Markdown', ...view.keyboard });
    await ctx.answerCbQuery();
  }

  async function sessionDelete(ctx) {
    const rc = await resolveClass(ctx);
    if (!rc.ok) return ctx.answerCbQuery(rc.reason);
    if (denied(ctx, rc, 'deleteSession')) return undefined;
    const recordIndex = parseInt(ctx.match[3], 10);
    ...
    const picked = scoped[recordIndex - 1];
    if (!picked) return ctx.answerCbQuery(TEXT.invalidRecordIndex);
    const res = await deleteSession(rc.cls.groupId, picked.id);
    if (!res || !res.ok) return ctx.answerCbQuery(OT.sessionDeleteFailed);
    // Re-render the class-level list (the per-type indices shift after a delete).
    const remaining = sessionsInSeries(await getAllSessions(rc.cls.groupId), 1);
    const items = remaining.map((session, i) => ({ session, recordIndex: i + 1 }));
    const view = renderAllSessions(rc.cls, items, 0, rc.caps, await classTz(rc.cls.groupId));
    await ctx.editMessageText(view.text, { parse_mode: 'Markdown', ...view.keyboard });
    return ctx.answerCbQuery(OT.sessionDeletedToast(picked.name));
  }
...
export function register(bot, storage) {
  ...
  bot.action(/^o:slink:(\d+):(\d+):(\d+)$/, h.studentLink);
  ...
  bot.action(/^o:tcat:(\d+):([a-zA-Z]+)$/, h.teacherCategory);
  bot.action(/^o:addteach:(\d+)$/, h.addTeacherPrompt);
  bot.action(/^o:addteachr:(\d+):([a-zA-Z]+)$/, h.addTeacherNamesPrompt);
  ...
  bot.action(/^o:allsess:(\d+):(\d+)$/, h.allSessions);
  bot.action(/^o:sdel:(\d+):(\d+):(\d+)$/, h.sessionDeleteConfirm);
  bot.action(/^o:sdelx:(\d+):(\d+):(\d+)$/, h.sessionDelete);
