...
import { beginForceReplyAwaiting, logTelegramError } from '../../helpers.js';
...
// current display index. The manage widget orders names by rawSessionNames
// (insertion / recitation-queue order); the shared resolver in
// sessionParticipants maps the token (member `u<id>` / guest `g<i>` / legacy
// bare index) back to `{ name, index }` within that order.
function resolveManageTarget(session, master, token) {
  return participants.resolveToken(session, rawSessionNames(session, master), token);
}
...
export function createHandlers({
  ...
  const { getMaster, getSession, saveSession, saveParticipant, setGroupRecitationPageCounter, setReplyPrompt, getPageProgress } = storage;
  ...
  async function editPage(ctx) {
    ...
    const prompt = await beginForceReplyAwaiting(ctx, {
      ...
      record: { action: 'editPage', memberName: name, memberIndex: i, sessionType: activeType },
      sendPrompt: () => ctx.reply(TEXT.editPagePrompt(name), {
        parse_mode: 'Markdown',
  ...
  async function editVerse(ctx) {
    ...
    const prompt = await beginForceReplyAwaiting(ctx, {
      setReplyPrompt, groupId,
      record: { action: 'editVerse', memberName: name, memberIndex: i, sessionType: activeType },
  ...
  async function addGuest(ctx) {
    ...
    const prompt = await beginForceReplyAwaiting(ctx, {
      setReplyPrompt, groupId,
      record: { action: 'addGuest', sessionType: activeType },
      sendPrompt: () => ctx.reply(TEXT.addGuestPrompt, {
        parse_mode: 'Markdown',
  ...
  async function editName(ctx) {
    ...
    const prompt = await beginForceReplyAwaiting(ctx, {
      setReplyPrompt, groupId,
      record: { action: 'editSessionName', managePage: safePage, sessionType: activeType },
      sendPrompt: () => ctx.reply(TEXT.editSessionNamePrompt(session.name), {
        parse_mode: 'Markdown',
  ...
  return { choose, pick, setStatus, setCall, editPage, editVerse, backToIndex, back, page, noop, refreshPage, refresh, hide, dismiss, addGuest, editName };
}
