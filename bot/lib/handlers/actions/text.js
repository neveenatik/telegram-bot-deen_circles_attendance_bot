...
import { renderClassHome, renderRoster, renderTeachers, renderTeacherMenu, renderManagersMenu, renderManagerMenu, renderTrainingGroups, renderTrainingGroupMenu, offlineOrderNames } from './offline.js';
...
// Rebuild the shared-editor nav when refreshing a panel from a force-reply.
// Offline classes (ns 'o') order members by roster list number, so the refreshed
// keyboard must encode the SAME positional tokens the offline editor resolves
// against — otherwise it would use alphabetical indices and the next tap would
// hit the wrong student (see offlineOrderNames).
function editorNav(pending, fallbackGref) {
  const ns = pending.ns || 'h';
  const nav = { ns, gref: pending.gref ?? fallbackGref };
  if (ns === 'o') nav.order = offlineOrderNames;
  return nav;
}
...
export function createHandlers({ storage, telegram }) {
  ...
  async function onText(ctx, next) {
    ...
    if (pending.action === 'historyEditVerse') {
      ...
      const nav = editorNav(pending, groupId);
