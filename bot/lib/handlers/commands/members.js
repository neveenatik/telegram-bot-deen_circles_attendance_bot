...
export function createHandlers({ storage, telegram, refreshSessionWidget = defaultRefreshSessionWidget }) {
  const { getMaster, saveMaster, getActiveSession, saveSession, getPendingRegistrations, savePendingRegistrations, getClassTimezone } = storage;
  ...
  async function pendingstudents(ctx) {
    ...
    try {
      await telegram.sendMessage(ctx.from.id, pendingStudentsText(pending, 0, tz), {
        parse_mode: 'Markdown',
