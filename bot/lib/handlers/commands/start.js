...
export function createHandlers({
  ...
  async function startSession(ctx, type, initializeAttendance, defaultName, options = {}) {
    ...
    const participants = initializeAttendance
      ? Object.fromEntries(master.members.map((m) => [
          m.name,
          { name: m.name, memberId: String(m.userId), status: null, called: null, ...(m.listNumber != null ? { listNumber: m.listNumber } : {}) },
        ]))
      : {};
    ...
    const sent = await ctx.replyWithMarkdown(sessionText(session, master), sessionKb(session));
    session.messageId = sent.message_id;
    await saveSession(groupId, type, session);
    await refreshSessionWidget(telegram, session, master, async () => saveSession(groupId, type, session));
    try {
  ...
  async function startsecondarylist(ctx) {
    // Recitation-correction lists start EMPTY: students self-register by tapping
    // a status (walk-ins are queued as pending for later approval, exactly like
    // the main session), and the list fills in registration order.
    return startSession(ctx, 'registeredSecondary', false);
  }
