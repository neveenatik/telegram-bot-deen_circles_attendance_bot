...
const VALID_TYPES = ['courseteacher', 'trainingteacher', 'recitationteacher', 'homeworkteacher'];
...
function teacherListText(teachers) {
  ...
  // A teacher may hold several roles, so she is listed under each role she has.
  for (const type of VALID_TYPES) {
    const inType = teachers.filter((t) => Array.isArray(t.types) && t.types.includes(type));
...
export function createHandlers({ storage }) {
  ...
  async function addteacherreply(ctx) {
    if (!await isAdmin(ctx)) return replyEphemeral(ctx, TEXT.adminOnly);
    ...
    teachers.push({ userId, name, types: [type] });
  ...
  async function assignteacher(ctx) {
    ...
    for (const entry of entries) {
      ...
      // Add the role to her set (a teacher may hold several roles at once).
      const set = new Set(Array.isArray(teacher.types) ? teacher.types : []);
      set.add(type);
      teacher.types = VALID_TYPES.filter((t) => set.has(t));
  ...
  async function tagteachers(ctx) {
    if (!await isAdmin(ctx)) return replyEphemeral(ctx, TEXT.adminOnly);
    ...
    const teachers = await getTeachers(groupId);
    const filtered = type ? teachers.filter(t => Array.isArray(t.types) && t.types.includes(type)) : teachers;
  ...
  return { addteacher, addteacherreply, removeteacher, assignteacher, listteachers, tagteachers };
}

export function register(bot, storage) {
  const h = createHandlers({ storage });
  bot.command('addteacher', h.addteacher);
  bot.command('addteacherreply', h.addteacherreply);
  bot.command('removeteacher', h.removeteacher);
  bot.command('assignteacher', h.assignteacher);
  bot.command('listteachers', h.listteachers);
  bot.command('tagteachers', h.tagteachers);
}
